...
export function Step2Mapping({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3.5">
        <div className="flex gap-2.5">
          ...
          <p className="text-xs text-amber-800">
            Sélectionnez maintenant le nouveau composant que vous souhaitez insérer, puis associez chaque calque de l'ancien composant à celui correspondant dans le nouveau.
          </p>
        </div>
      </div>
      ...
      <div>
        <label className="text-xs text-neutral-500 mb-2 block">Composant cible</label>
        {!newComponent ? (
          <button
          ...
          >
            {isSelectingNew ? (
              ...
              <>
                ...
                <span className="text-xs text-neutral-400">Sélectionnez le nouveau composant (ou une instance) dans Figma, puis cliquez ici</span>
              </>
            )}
          </button>
        ) : (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500">{newComponent.layers.length} layers disponibles</p>
                </div>
              </div>
      ...
      {newComponent && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs text-neutral-500">Associations ({mappedCount}/{mappings.length})</label>
            {allMapped && (
            ...
            {mappings.map((mapping) => {
              ...
              return (
                <div key={mapping.id}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isMapped ? (
                          <div className="flex items-center gap-2 flex-1 min-w-0 bg-emerald-50 border border-emerald-200 rounded-md px-2.5 py-2">
                            ...
                            <p className="text-xs text-neutral-900 truncate flex-1">{mapping.targetLayer!.name}</p>
                  ...
                  {/* Target picker — layers du VRAI nouveau composant */}
                  {isActive && (
                    <div className="mt-1.5 border border-blue-200 rounded-lg bg-blue-50/50 overflow-hidden">
                      <div className="px-3 py-2 bg-blue-100/60 border-b border-blue-200 flex items-center justify-between">
                        <span className="text-xs text-blue-800">Associer « {mapping.sourceLayer.name} » à :</span>
                          ...
                          .map((layer) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {layer.componentName && <span className="text-[10px] text-neutral-400">{layer.componentName}</span>}
                                {alreadyUsed && <span className="text-[10px] text-blue-500">associé</span>}
                              </button>
                            );
                          })}
                        {newComponent.layers.filter((nl) => nl.type === mapping.sourceLayer.type).length === 0 && (
                          <p className="text-xs text-neutral-400 p-2 text-center">Aucun layer compatible ({mapping.sourceLayer.type})</p>
                        )}
                      </div>
                    </div>
                  )}
